...
<body>
    ...
    <div class="container" style="margin-top: 65px;">
        <div class="col-md-7" style="float: none; margin: 0 auto;">
            <div class="form-area">
                <form role="form" action="/maintenance" enctype="multipart/form-data" method="POST">
                    @csrf
                    <br style="clear: both">
                    <h3 style="margin-bottom: 25px; text-align: center; font-size: 30px;"> Silahkan Memasukan Detail
                        ...
                        <div class="form-group">
                            ...
                            <select name="mobil_id" class="form-control" aria-label="Default select example">
                                <option selected hidden>Mobil Select</option>
                                @foreach ($mobil as $mobilx)
                                    <option value="{{ $mobilx->id }}">{{ $mobilx->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        <textarea class="form-control" id="jenis_kerusakan" name="jenis_kerusakan"
                            placeholder="Jenis Kerusakan" required></textarea>
                    </div>

                    <div class="form-group">
                        <input type="number" class="form-control" id="perbaikan_price" name="perbaikan_price"
                            placeholder="Biaya Perbaikan" required>
                    </div>
                    ...
                    <table class="table table-striped">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th width="20%"> Biaya Perbaikan </th>
                        ...
                        <tbody>
                            @foreach ($maintenance as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->tbl_mobil->nama }}</td>
                                    <td>{{ $item->jenis_kerusakan }}</td>
                                    <td>{{ $item->biaya_perbaikan }}</td>
                                    <td>{{ $item->tbl_akun->nama }}</td>
                                    <td>
                                        <form role="form" action="/maintenance/{{ $item->id }}"  method="POST">
                                            @csrf 
                                            @method('delete')
